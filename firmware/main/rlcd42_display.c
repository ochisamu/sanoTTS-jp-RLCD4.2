...
static void draw_hline(int x, int y, int width, int color)
{
    fill_rectangle(x, y, width, 1, color);
}
...
{
    fill_rectangle(137, 181, 126, 55, PIXEL_WHITE);
    if (!speaking) {
        fill_rectangle(165, 207, 70, 4, PIXEL_BLACK);
        return;
    }

    const int height = 10 + (int)level * 34 / 255;
    const int y = 208 - height / 2;
    fill_rectangle(160, y, 80, height, PIXEL_BLACK);
    if (height > 8) fill_rectangle(166, y + 4, 68, height - 8, PIXEL_WHITE);
}

static void draw_status(const char *status, const char *detail)
{
    fill_rectangle(0, 244, RLCD_WIDTH, RLCD_HEIGHT - 244, PIXEL_WHITE);
    draw_hline(24, 244, RLCD_WIDTH - 48, PIXEL_BLACK);
    draw_text_centered(254, status, 2, PIXEL_BLACK);
    if (detail && *detail) draw_text_centered(280, detail, 1, PIXEL_BLACK);
}
...
static void draw_current_status(void)
{
    ...
    switch (s_ui_state) {
    case UI_READY:
        (void)snprintf(detail, sizeof detail, "%s / KEY DEMO", s_battery_summary);
        draw_status("TTS READY", detail);
        break;
    case UI_SPEAKING:
    ...
    case UI_PROBE:
        ...
        break;
    case UI_ERROR:
        draw_status("TTS ERROR", s_error_detail);
        break;
    case UI_STARTING:
    default:
...
static void draw_base_screen(void)
{
    ...
    draw_text_centered(18, "SANO TTS", 5, PIXEL_BLACK);
    draw_text_centered(66, "RLCD4.2", 3, PIXEL_BLACK);
    draw_hline(24, 100, RLCD_WIDTH - 48, PIXEL_BLACK);

    /* A deliberately simple face remains clear on the two-level panel. */
    fill_rectangle(83, 130, 86, 15, PIXEL_BLACK);
    fill_rectangle(231, 130, 86, 15, PIXEL_BLACK);
    fill_rectangle(116, 126, 18, 23, PIXEL_WHITE);
    fill_rectangle(264, 126, 18, 23, PIXEL_WHITE);
    draw_mouth(0, false);
